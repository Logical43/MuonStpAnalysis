// version 2019 of IMuonTargetTrajectoryAnalyzer, no tagger included
...
bool COMET::IMuonTargetTrajectoryAnalyzer::AnalyzeTrajectory(
    ...
    if(isElectron){
        ...
        if(momentum.P()>103.6 && momentum.P()<106) ++fAllTrajData.nHighEElectrons;
    }
...
bool COMET::IMuonTargetTrajectoryAnalyzer::FinalizeHistograms(){
    ...
    hSummaryData->Fill("High Energy Electrons",fAllTrajData.nHighEElectrons);
    hSummaryData->SetDrawStyle("h text30");
    AddHistogram(hSummaryData);


    // writes the parameters into txt files
    double totalStoppingRate = (double)nStoppedMuonsAll/(double)(nStoppedMuonsAll+nPassedMuonAll);
    int nHEElectrons = fAllTrajData.nHighEElectrons;
    ofstream SRfile;
    SRfile.open ("stoppingRate.txt", std::ios_base::app);
    SRfile << totalStoppingRate << "\n";
    SRfile.close();

    ofstream HEfile;
    HEfile.open("highEElectrons.txt",std::ios_base::app);
    HEfile << nHEElectrons << "\n";
    HEfile.close();
